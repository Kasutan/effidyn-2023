...
if(have_rows('cols')) : 
	...
	printf('<section class="acf valeurs-cols %s">', $className);
		...
		if($titre_section) printf('<h2 class="titre-section decor-trait-simple">%s</h2>',$titre_section);
			...
			while(have_rows('cols')) : the_row();
				...
				$sous_titre=wp_kses_post( get_sub_field('sous_titre') );
				$texte=wp_kses_post( get_sub_field('texte') );
				printf('<div class="col"><h3 class="titre">%s</h3><p class="sous-titre">%s</p><div class="texte">%s</div></div>',$titre,$sous_titre,$texte);
